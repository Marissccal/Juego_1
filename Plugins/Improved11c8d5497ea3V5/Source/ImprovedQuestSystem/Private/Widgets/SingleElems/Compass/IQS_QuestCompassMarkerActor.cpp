#include "Widgets/SingleElems/Compass/IQS_QuestCompassMarkerActor.h"

// Copyright Evgeniy w4ee44@gmail.com

void UIQS_QuestCompassMarkerActor::UpdateStateCpp(const FVector& PlayerLoc, const FRotator& PlayerRot, const FVector& PlayerMinimapLoc)
{
	...
	if (IsValid(ActorTargetCpp))
	{
		...
		PlayerContr->ProjectWorldLocationToScreen(PlayerLoc, ScreenPositionCpp, true);
...
#undef LOCTEXT_NAMESPACE
		...
		FVector2D Transition = CalcRotationCpp(PlayerContr->GetControlRotation(), UKismetMathLibrary::FindLookAtRotation(PlayerLoc, ActorTargetCpp->GetActorLocation()), IsC, IsSight);
		if (IsSight)
		{
	...
	else
	{
		if (!IsRemovedCpp)
		{
			RemoveQuestMarkByActor_BP();
			IsRemovedCpp = true;
		}
	}
}

void UIQS_QuestCompassMarkerActor::UpdateStateWithCheckCpp(const FVector& PlayerLoc, const FRotator& PlayerRot, const FVector& PlayerMinimapLoc)
{
	APlayerController* PlayerContr = GetOwningPlayer();
	if (IsValid(ActorTargetCpp))
	{
		MarkerDistanceToPlayerCpp = FVector::Dist2D(PlayerLoc, ActorTargetCpp->GetActorLocation()) * 0.01f;
		if (MarkerDistanceToPlayerCpp < DistanceToHideCpp)
		{
			SetVisibility(ESlateVisibility::Hidden);
		}
		else
...
void UIQS_QuestCompassMarkerActor::CAC_Cpp(float X, float X1, ECompassComparisonResult& CompRes, float& DX, float& DY)
{
...
void UIQS_QuestCompassMarkerActor::CC_Cpp(float X, float Y, ECompassComparisonResult& CompRes, float& DX, float& DY)
{
...
float UIQS_QuestCompassMarkerActor::CalcD(float A, float B, bool Cl)
{
...
FVector2D UIQS_QuestCompassMarkerActor::CalcRotationCpp(FRotator X, FRotator Y, bool& IsC, bool& IsSight)
{
...
float UIQS_QuestCompassMarkerActor::DegreeRotCpp(float In)
{
	if (In < 0.0f)
	{
